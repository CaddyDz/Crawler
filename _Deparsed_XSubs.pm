...
sub _Disconnect($$) ;
sub _EndTransaction($$) ;
sub _EstablishContext($$$) ;
sub _GetStatusChange($$$) ;
sub _ListReaders($$) ;
sub _LoadPCSCLibrary() ;
sub _Reconnect($$$$) ;
sub _ReleaseContext($) ;
sub _Status($) ;
sub _Transmit($$$) ;
...
sub pcap_close;
sub pcap_compile;
sub pcap_compile_nopcap;
sub pcap_createsrcstr;
sub pcap_datalink;
sub pcap_datalink_name_to_val;
sub pcap_datalink_val_to_description;
sub pcap_datalink_val_to_name;
sub pcap_dispatch;
sub pcap_dump;
sub pcap_dump_close;
sub pcap_dump_file;
...
sub pcap_fileno;
sub pcap_freecode;
sub pcap_get_selectable_fd;
sub pcap_geterr;
sub pcap_getevent;
sub pcap_getnonblock;
sub pcap_is_swapped;
sub pcap_lib_version;
sub pcap_lookupdev;
sub pcap_lookupnet;
sub pcap_loop;
sub pcap_major_version;
sub pcap_minor_version;
sub pcap_next;
sub pcap_next_ex;
sub pcap_offline_filter;
sub pcap_open;
sub pcap_open_dead;
sub pcap_open_live;
sub pcap_open_offline;
sub pcap_parsesrcstr;
sub pcap_perror;
sub pcap_sendpacket;
sub pcap_sendqueue_alloc;
sub pcap_sendqueue_queue;
sub pcap_sendqueue_transmit;
sub pcap_set_datalink;
sub pcap_setbuff;
sub pcap_setfilter;
sub pcap_setmintocopy;
sub pcap_setmode;
sub pcap_setnonblock;
sub pcap_setuserbuffer;
sub pcap_snapshot;
sub pcap_stats;
sub pcap_strerror;
sub perl_settings;
...
sub file($) ;
sub fileno($) ;
sub generic_pkt_creat($) ;
sub generic_pkt_parse($) ;
sub geterr($) ;
sub host_to_ip($) ;
sub icmp_pkt_creat($) ;
sub icmp_pkt_parse($) ;
